...
class ExecuteSql:
    def __init__(self, params):
        self.submit_sql_uuid = params['submit_sql_uuid']
        self.split_sql_file = params['split_sql_file']
        self.inc_backup = params['inc_backup']
        self.inc_ignore_warn = params['inc_ignore_warn']
        self.inc_ignore_err = params['inc_ignore_err']
        self.inc_execute_sleep_ms = params['inc_execute_sleep_ms']
        self.login_user_name= 'gaochao'

    def task_run(self):
        logger.info("[工单:%s],开始执行......", self.split_sql_file)
        data = audit_sql_dao.get_master_info_by_split_sql_file_path_dao(self.split_sql_file)
        ...
        try:
            ret = inception.execute_sql(des_master_ip, des_master_port, inception_backup,
                                        inception_check_ignore_warning, inception_execute_ignore_error, execute_sql,
                                        submit_sql_uuid, osc_config_sql)
        ...
        # 更改sql_submit_info状态
        try:
            get_all_sql_split_status = "select execute_status from sql_execute_split where submit_sql_uuid='{}'".format(
                submit_sql_uuid)
            ...
            sql_update_execute_max_code = "update sql_submit_info set execute_status={} where submit_sql_uuid='{}'".format(
                max_code, submit_sql_uuid)
            ...
            try:
                ...
                for row in results:
                    ...
                    insert_execute_results_sql = """insert into sql_execute_results(submit_sql_uuid,inception_id,inception_stage,inception_error_level,inception_error_message,inception_sql,inception_affected_rows,inception_execute_time,split_sql_file_path) 
                                                                                    values('{}',{},'{}',{},'{}','{}',{},'{}','{}')
                    """.format(submit_sql_uuid, inception_id, inception_stage, inception_error_level,
                               pymysql.escape_string(inception_error_message), pymysql.escape_string(inception_sql),
                               inception_affected_rows, inception_execute_time, split_sql_file_path)

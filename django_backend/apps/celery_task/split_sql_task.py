...
class AsyncSplitSql:
    ...
    def task_run(self):
        # 发送拆分任务L---->推送到celery----->获取到任务------->预检查----->发送审核工具拆分----->处理审核工具拆分结果----->标记工单状态
        common.mark_celery_task(self.submit_sql_uuid, 1)
        common.audit_sql_log(self.submit_sql_uuid, 0, "获取到拆分任")
        try:
            self.send_inception()
    ...
    def send_inception(self):
        ...
        if split_ret['status'] == 'ok':
            ...
            common.audit_sql_log(self.submit_sql_uuid, 0, "审核工具拆分完成")
        else:
            raise Exception("审核工具拆分失败%s" % split_ret['message'])
    ...
    def write_split_sql_to_new_file(self):
        ...
        try:
            ...
            common.audit_sql_log(self.submit_sql_uuid, 0, "拆分SQL写入完成")
        except Exception as e:

...
# 工单-->权限申请-->查看已有用户信息-->使用private_user表
def get_private_user_info_func(request):
    ...
    if request.method == 'GET':
        sql = """select id as 'key',
                    ...
                    case dba_check_result when 1 then '通过' else '未审核' end dba_check_result,
...
def execute_order_func(request):
    ...
    create_user_info_sql = "select db_master_ip, db_master_port, user_name, user_host, privileges, db_name, tb_name from privilege_request_info where order_uuid = '{}'".format(order_uuid)
    ...
    try:
        ...
        order_status_sql = "update privilege_request_info set status=2 where order_uuid='{}'".format(order_uuid)
...
def privileges_create_user_info_func(request):
    ...
    if request.method == 'POST':
        ...
        grant_person_name = request_body['params']['grant_person_name'].strip()  # 申请人
        grant_request_type = 0  # 工单类型为创建新用户
        grant_department = '研发'  # 部门
        grant_leader = '宋小宝'
        grant_dba = '宋大宝'

        # grant_department = request_body['params']['grant_department'].strip()  # 部门
        # grant_leader = request_body['params']['grant_leader'].strip()  # 业务leader
        # grant_dba = request_body['params']['grant_dba'].strip()  # DBA
        ...
        try:
            ...
            if not cur.fetchall():
                ...
                add_user_info_sql = "insert into devops.privilege_request_info(order_uuid,person_name,request_type,department,leader,dba,leader_check_result,dba_check_result,db_master_ip,db_master_port,user_name,user_host,privileges,db_name,tb_name,status,ctime,utime) values ('{}','{}','{}','{}','{}','{}',0,0,'{}',{},'{}','{}','{}','{}','{}',1,now(),now())".format(str_order_uuid,grant_person_name,int(grant_request_type),grant_department,grant_leader,grant_dba,grant_db_master_ip,int(grant_db_master_port),grant_user_name,grant_user_host,grant_privileges,grant_database,grant_table)
...
def privileges_extend_info_func(request):
    ...
    if request.method == 'POST':
        ...
        grant_user_name = request_body['params']['grant_user_name'].strip()  # 用户名
        grant_person_name = request_body['params']['grant_person_name'].strip()  # 申请人
        grant_request_type = request_body['params']['grant_request_type'].strip()  # 工单类型
        grant_department = request_body['params']['grant_department'].strip()  # 部门
        grant_leader = request_body['params']['grant_leader'].strip()  # 业务leader
        grant_dba = request_body['params']['grant_dba'].strip()  # DBA
        ...
        try:
            ...
            if rows:
                ...
                str_order_uuid = str(uuid.uuid4())
                add_order_info_sql = """insert into devops.privilege_request_info(order_uuid,person_name,request_type,department,leader,dba,leader_check_result,dba_check_result,db_master_ip,db_master_port,user_name,user_host,privileges,db_name,tb_name,status,ctime,utime) 
                                            values ('{}','{}','{}','{}','{}','{}',0,0,'{}',{},'{}','{}','{}','{}','*',1,now(),now())""".format(str_order_uuid,grant_person_name,int(grant_request_type),grant_department,grant_leader,grant_dba,grant_db_master_ip,int(grant_db_master_port),grant_user_name,str_ip_list,grant_privileges,str_database_list)
...
def check_order_func(request):
    ...
    print(request_body['params'])
    order_uuid = request_body['params']["order_uuid"]
    leader_check_result = request_body['params']["leader_check_result"]
    if leader_check_result=='通过':

...
def adhoc():
    ...
    context.CLIARGS = ImmutableDict(connection='smart', module_path=['/to/mymodules', '/usr/share/ansible'], forks=10, become=None,
                                    become_method=None, become_user=None, check=False, diff=False,remote_user='root',verbosity=0)
    # required for
    ...
    play_source = dict(
        ...
        gather_facts='no',
        tasks=[
            dict(action=dict(module='shell', args='touch /tmp/2.txt'), register='shell_out'),
    ...
    for host, result in results_callback.host_ok.items():
        print('{0} >>> {1} \n{2}'.format(host, result._result['cmd'],result._result['stdout']))

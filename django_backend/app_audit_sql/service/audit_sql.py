...
def check_sql(submit_type, check_sql_info, cluster_name, instance_name, check_sql_uuid, check_type, user_offer_rollback_sql):
    ...
    :param cluster_name:
    :param instance_name:
    :param submit_sql_uuid:None代表预审核、str代表修改已提交的SQL
    :param check_type:None代表预审核、recheck_sql代表修改已提交的SQL
    :return:
    """
    ...
    if check_sql_uuid is None: check_sql_uuid = str(uuid.uuid4())
    if check_type is None: check_type = "check_sql"
    check_user = "gaochao"
    try:
        """
...
def get_view_sql_by_uuid(submit_sql_uuid):
    ...
    try:
        ...
        with open("{}/{}".format(upload_base_path, sql_file_path), "rb") as f1:
            sql_text = f1.read().decode('utf-8')
        with open("{}/{}".format(upload_base_path, rollback_sql_file_path), "rb") as f2:
...
def get_submit_split_sql_by_file_path(split_sql_file_path):
    try:
        with open("{}/{}".format(upload_base_path, split_sql_file_path), "rb") as f:
            data = f.read()
...
class SubmitSql:
    ...
    def write_sql_to_file(self):
        ...
        # 确定存放路径
        upload_path = upload_base_path + '/' + now_date
...
class PassSubmitSql:
    ...
    def split_sql(self):
        try:
            ...
            如果任务注册成功则返回task_res,否则出现异常
            """
            task_res = inception_split.delay(self.submit_sql_uuid, self.ticket_info, self.cal_des_ip, self.cal_des_port,
                                            self.check_status, self.check_comment, self.login_user_name,
                                            self.login_user_name_role)
            self.celery_id = task_res.id
            logger.info("celery发送审核任务成功返回task_id:%s,工单id:%s" % (task_res.id, self.submit_sql_uuid))
        except Exception as e:
            logger.exception("发送任务失败:%s" % str(e))
...
def recreate_sql(split_sql_file_path, recreate_sql_flag):
    ...
    rerun_file_path = "{}/{}".format(upload_base_path, rerun_file)
    try:

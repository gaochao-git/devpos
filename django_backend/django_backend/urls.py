...
urlpatterns = [
    ...
    # sql审核begin
    path('api/v1/service/ticket/audit_sql/get_submit_sql_info/', audit_sql_controller.GetSqlSubmitSqlInfoCrotroller.as_view()), # 页面获取所有工单列表
    ...
    path('api/get_master_ip/', audit_sql_controller.GetMasterIpController.as_view()), # sql审核--获取主库ip
    path('api/get_cluster_name/', audit_sql_controller.GetClusterNameCrotroller.as_view()), # sql审核--根据cluster_name输入框自动补全
    ...
    path('api/v1/service/ticket/audit_sql/get_pre_check_result/', audit_sql_controller.GetPreCheckResultCrotroller.as_view()),  # 生成用id切割的SQL用来删除或者更新数据,防止大事物
